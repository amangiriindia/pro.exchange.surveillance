...
class BulkOrderDetailsBloc extends Bloc<BulkOrderDetailsEvent, BulkOrderDetailsState> {
  final GetBulkOrderDetails getBulkOrderDetails;

  BulkOrderDetailsBloc({required this.getBulkOrderDetails}) : super(BulkOrderDetailsInitial()) {
    on<LoadBulkOrderDetails>(_onLoadBulkOrderDetails);
  }

  void _onLoadBulkOrderDetails(LoadBulkOrderDetails event, Emitter<BulkOrderDetailsState> emit) async {
    emit(BulkOrderDetailsLoading());
    final result = await getBulkOrderDetails(event.symbol);
    result.fold(
